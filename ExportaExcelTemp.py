...
for i1, Agrupacion in ListaDeAgrup.iterrows():
    ...
    for i2, GxBloque in ListaGxBloques.iterrows():
        ...
        #         UPDATE	dbo.Fechas
        #         SET		FACT_EMITIDA = X.NumReg
        #         FROM(
        #                 SELECT	m.MesDevengado,
        #                         CASE
        #                             WHEN COUNT(*) > 0 THEN 1
        #                             ELSE 0
        #                         END AS NumReg
        #                 FROM	dbo.SIGGE_Facturacion f
        #                         LEFT JOIN dbo.SIGGE_Diccionario d ON f.Nombre = d.SIGGE
        #                         LEFT JOIN dbo.AgrupacionSIGGE e ON f.ClaveEEDD = e.NomAgrupacion
        #                         LEFT JOIN dbo.Agrupacion a ON e.IdAgrupacion = a.IdAgrupacion
        #                         LEFT JOIN dbo.SIGGE_MesDevengado2 m ON f.Glosa = m.Glosa
        #                         LEFT JOIN dbo.SIGGE_DiccionarioConceptos dc ON f.Concepto = dc.Concepto
        #                 WHERE	f.Empresa LIKE '%{GX_Sigge}%'
        #                         AND f.Bloque = '{Bloque}'
        #                         AND a.NomAgrupacion = '{NomAgrupacion}'
        #                         --AND dc.Agrupacion IN (|Agrupador|)
        #                 GROUP BY m.MesDevengado
        #         ) X
        #         WHERE	Fechas.Fecha = X.MesDevengado

        #         UPDATE	dbo.Fechas
        #         SET		FACT_EST_EFACT = X.NumReg
        #         FROM(
        #                 SELECT	c.Fecha,
        #                         CASE
        #                             WHEN COUNT(*) > 0 THEN 1
        ...
        #                         LEFT JOIN dbo.{AbrevCliente}_AgrupacionEFACT e ON DX+'|'+NomEmpresaDistribuidora+'|'+c.GX+'|'+NomSuministrador+'|'+CodigoContrato+'|'+Contrato = e.Tag
        #                         LEFT JOIN dbo.Agrupacion a ON e.IdAgrupacion = a.IdAgrupacion
        #                         LEFT JOIN dbo.LAP_PrecioGxBloqueBarraVigente d
        #                             ON	d.Generador = '{Empresa}'
        #                                 AND d.Bloque = '{Bloque}'
        ...
        #                 GROUP BY c.Fecha
        #         ) X
        #         WHERE	Fechas.Fecha = X.Fecha

        #         --Update del CEN falta

        #         UPDATE	dbo.Fechas
        #         SET		EFACT_CNE = X.NumReg
        #         FROM(
        #                 SELECT	c.Fecha,
        #                         CASE
        #                             WHEN COUNT(*) > 0 THEN 1
        ...
        #     Mapa_Data_SQL = pandas.read_sql(f"""
        #         SELECT	Num,
        #                 YEAR(Fecha) AS Anho,
        #                 MONTH(Fecha) AS Mes,
        #                 CONVERT(VARCHAR(100), Fecha, 105) AS Fecha,
        #                 FACT_EMITIDA,
        #                 FACT_EST_EFACT,
        #                 '' AS Sep1, --FACT_EST_CEN
        #                 EFACT_CNE,
        #                 '' AS Sep2, --EFACT_EST_CEN
        #                 CASE
        #                     WHEN FACT_EMITIDA = 1 THEN 'FACT_EMITIDA'
        #                     WHEN FACT_EST_EFACT = 1 THEN 'FACT_EST_EFACT'
        #                     --WHEN FACT_EST_CEN = 1 THEN 'FACT_EST_CEN'
        #                     ELSE 'SIN_DATA'
        #                 END AS OrigenData1,
        #                 CASE
        #                     WHEN EFACT_CNE = 1 THEN 'EFACT_CNE'
        #                     --WHEN EFACT_EST_CEN = 1 THEN 'EFACT_EST_CEN'
        #                     ELSE 'SIN_DATA'
        #                 END AS OrigenData2
        #         FROM	dbo.Fechas
        #     """, engine)
        ...
        if( IdCliente == 1 ):
            ...
            FacturacionReal = pandas.read_sql(f"""
                SELECT	f.Empresa,
                        f.MesDevengado,
                        f.IdAgrupacion,
                        a.NomAgrupacion,
                        ...
                        f.CodigoDeArticulo,
                        f.NumDeFolio,
                        SUM(f.TotalDoc) AS MontoCLP
                FROM	dbo.LAP_Facturacion f
        ...
        if( IdCliente == 1 ):
            ...
            FacturacionReal.to_excel(writer, sheet_name='FACT_REAL', index=False)
